...
use windows::Win32::UI::Input::KeyboardAndMouse::{
  RegisterHotKey, SendInput, UnregisterHotKey, INPUT, INPUT_KEYBOARD, KEYBD_EVENT_FLAGS,
  KEYEVENTF_KEYUP, KEYEVENTF_UNICODE, MOD_ALT, MOD_CONTROL, MOD_NOREPEAT, MOD_SHIFT, VIRTUAL_KEY,
};
use windows::Win32::UI::WindowsAndMessaging::{
  CreateWindowExW, DefWindowProcW, DispatchMessageW, PeekMessageW, RegisterClassW,
  TranslateMessage, CW_USEDEFAULT, HMENU, PM_REMOVE, WM_HOTKEY, WM_QUIT, WNDCLASSW, WS_EX_LAYERED,
...
lazy_static! {
  ...
  static ref GLOBAL_HOTKEY_CHANNEL: (Sender<GlobalHotkeyMessage>, Receiver<GlobalHotkeyMessage>) =
    unbounded();
  static ref GLOBAL_HOTKEY_CALLBACKS: Mutex<HashMap<u32, Vec<ThreadsafeFunction<()>>>> =
        ...
        while let Ok(msg) = GLOBAL_HOTKEY_CHANNEL.1.try_recv() {
          match msg {
            GlobalHotkeyMessage::Register(hotkey) => {
              ...
              hotkey.mods.iter().for_each(|it| match it {
...
impl Keyboard {
  ...
  pub fn register_hotkey(mods: Vec<Modifiers>, key: Key, callback: JsFunction) -> u32 {
    ...
    let tsfn: ThreadsafeFunction<()> = callback
      .create_threadsafe_function(0, |_ctx| Ok(vec![0]))
      .unwrap();
    ...
    GLOBAL_HOTKEY_CHANNEL
      .0
      .send(GlobalHotkeyMessage::Register(hotkey))
      .unwrap();
  ...
  pub fn unregister_hotkey(id: u32) {
    ...
    GLOBAL_HOTKEY_CHANNEL
      .0
      .send(GlobalHotkeyMessage::Unregister(id))
      .unwrap();
  }
}
