use crate::utils::{
  ...
  bigint_to_u64, bigint_to_u8, bigint_to_usize, decode_wide, handle_result,
};
...
use windows::Win32::System::Threading::{
  OpenProcess, PROCESS_ACCESS_RIGHTS, PROCESS_ALL_ACCESS, PROCESS_CREATE_PROCESS,
  PROCESS_CREATE_THREAD, PROCESS_DELETE, PROCESS_DUP_HANDLE, PROCESS_QUERY_INFORMATION,
  PROCESS_QUERY_LIMITED_INFORMATION, PROCESS_READ_CONTROL, PROCESS_SET_INFORMATION,
  PROCESS_SET_LIMITED_INFORMATION, PROCESS_SET_QUOTA, PROCESS_SET_SESSIONID, PROCESS_SYNCHRONIZE,
  PROCESS_TERMINATE, PROCESS_VM_OPERATION, PROCESS_VM_READ, PROCESS_VM_WRITE, PROCESS_WRITE_DAC,
  PROCESS_WRITE_OWNER,
};
...
fn read_memory_inner<T: Default>(
  handle: HANDLE,
  address: usize,
...
fn read_memory_chain_inner<T: Default>(
  ...
  Ok(read_memory_inner::<T>(
    handle,
    address + offsets[last_index],
  )?)
}

async fn read_memory<T: Default + Send + 'static>(handle: HANDLE, address: u64) -> Result<T>
where
  T: Send + 'static,
{
